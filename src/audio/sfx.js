...
// Все файлы нормализованы по пику одинаково, но по громкости они очень
// разные, поэтому цифры здесь так разбросаны: gain компенсирует RMS
// конкретного семпла, а не «делает погромче». В файлах баланс править
// нельзя — потеряется при следующей пересборке пака.
//
// Значения подобраны по воспринимаемой громкости, а не по RMS: слух
// интегрирует около 200 мс, и десятимиллисекундный щелчок при той же
// энергии звучит заметно тише длинного звука.
// Порядок от тихого к громкому:
//   ui-tap < deny-* < complete < take/undo < hint/post-added < cap/land < win
const SFX_GAINS = {
  take: 0.94,
  undo: 0.37,
  complete: 0.22,
